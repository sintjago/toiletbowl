...
function drawFlush(ctx, img, box, pose, rig) {
  ...
  if (pose.handle < 0.01 && lidAmt < 0.01) {
    ctx.restore();
    return;
  }

  const porcelain = sampleColor(img, 0.57, 0.4);
  const shade = sampleColor(img, 0.62, 0.48);
  ...
  if (lidAmt > 0.02) {
    ...
    ctx.fillStyle = "#1a3f48";
    ctx.fill();
    ctx.beginPath();
    ctx.ellipse(bx, by, brx * 0.96, bry * 0.9, bowl.tilt, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(46, 140, 168, ${0.45 + pose.level * 0.45})`;
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.ellipse(lid.x, lid.y + 3, lid.rx * 1.02, lid.ry + 2, lid.tilt, 0, Math.PI * 2);
    ctx.fillStyle = shade;
    ctx.fill();
    ctx.beginPath();
